...
const Resolved = (ticket) => {
  swal({
    ...
    customClass: {
      confirmButton:
        "inline-block px-5 py-3 m-0 mr-2 text-xs font-bold text-center text-white uppercase align-middle transition-all border-0 rounded-lg cursor-pointer ease-soft-in leading-pro tracking-tight-soft bg-primary shadow-soft-md bg-150 bg-x-25 hover:scale-102 active:opacity-85",
      cancelButton:
        "inline-block px-8 py-3 m-0 font-bold text-center uppercase align-middle transition-all bg-gray-200 border-0 rounded-lg cursor-pointer hover:scale-102 active:opacity-85 hover:shadow-soft-xs leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 text-slate-800",
    },
...
const filterText = async () => {
  router.get(
    route("ticket"),
    {
      ...
      page:
        props.requestParam && props.requestParam.page ? props.requestParam.page : null,
    },
    {
      preserveState: true,
      onError: (error) => {
...
<template>
  <Modal :show="showModal" :max-width="maxWidth" @close="hideReplyModal" class="w-full">
    <TicketReplyModal @close-modal="hideReplyModal" :ticket="selectedReplyTicket" />
  </Modal>
            ...
            <table class="table w-full" id="printTicketTable">
              ...
              <tbody>
                ...
                <tr v-for="ticket in tickets.data" :key="ticket.id">
                  ...
                  <td class="leading-normal text-sm truncate">
                    {{ truncateWords(ticket.problem_description, 15) }}
                  </td>
                  <td>
                    <Status :status="status(ticket.status)" />
                  </td>
                  <td class="leading-normal text-sm hide-on-print">
                    <button type="button" @click="showTicketView(ticket)">
                      <i class="fas fa-eye text-slate-400 dark:text-white/70"></i>
                    </button>
                    <button class="mx-2" type="button" @click="showReplyModal(ticket)">
                      <i class="fa-solid fa-reply"></i>
                    </button>
                    <button type="button" @click="Resolved(ticket)">
                      <i
                        class="fa-solid fa-circle-check text-slate-400 dark:text-white/70"
                      ></i>
                    </button>
                    <TicketViewModal
                    ...
                    >
                      ...
                      <template #ticket-details>
                        ...
                        >
                          ...
                          <div>
                            <p><b>Name</b> :{{ selectedTicket.customer_name }}</p>
                          </div>
                          <div>
                            <p><b>Email</b> :{{ selectedTicket.email }}</p>
                          </div>
                          <div>
                            <p><b>Phone Number</b> :{{ selectedTicket.phone_number }}</p>
                          </div>
                        </div>
                      </template>
                      <template #ticket-message>
                        <div class="w-full p-4 rounded-lg shadow-md">
                          ...
                          <div>
                            <p><b></b>{{ selectedTicket.problem_description }}</p>
                          </div>
                        </div>
                      </template>
                      <template #ticket-reply>
                        <div class="w-full p-4 rounded-lg shadow-md">
                          ...
                          <div
                            v-if="
                              selectedTicket.reply_ticket &&
                              selectedTicket.reply_ticket.length > 0
                            "
                          >
                            <div
                              v-for="reply_ticket in selectedTicket.reply_ticket"
                              :key="reply_ticket.id"
                            >
                              <p><b></b>{{ reply_ticket.message }}</p>
                            </div>
                          </div>
                          <div v-else>
