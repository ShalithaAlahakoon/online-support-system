...
const restoreConfirmation = (resolvedTicket) => {
  swal({
    ...
    customClass: {
      confirmButton:
        "inline-block mr-3  px-6 py-3 m-0 ml-2 text-xs font-bold text-center text-white uppercase align-middle transition-all border-0 rounded-lg cursor-pointer ease-soft-in leading-pro tracking-tight-soft bg-primary shadow-soft-md bg-150 bg-x-25 hover:scale-102 active:opacity-85",
      cancelButton:
        "inline-block px-6 py-3 m-0 font-bold text-center uppercase align-middle transition-all bg-gray-200 border-0 rounded-lg cursor-pointer hover:scale-102 active:opacity-85 hover:shadow-soft-xs leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 text-slate-800",
    },
...
const filterText = async () => {
  router.get(
    route("ticket.page.resolvedTicketIndex"),
    {
          ...
          <div class="overflow-x-auto table-responsive">
            <table class="table w-full" id="printresolvedTicketTable">
              ...
              <tbody>
                ...
                <tr
                  v-for="resolvedTicket in resolvedTickets.data"
                  :key="resolvedTicket.id"
                >
                  <td class="leading-normal text-sm">
                    {{ resolvedTicket.reference_number }}
                  </td>
                  <td class="leading-normal text-sm">
                    {{ resolvedTicket.customer_name }}
                  </td>
                  <td class="leading-normal text-sm">{{ resolvedTicket.email }}</td>
                  <td class="leading-normal text-sm">
                    {{ resolvedTicket.phone_number }}
                  </td>
                  <td class="leading-normal text-sm truncate">
                    {{ truncateWords(resolvedTicket.problem_description, 15) }}
                  </td>
                  <td>
                    <button
                      type="button"
                      class="mx-4"
                      @click="restoreConfirmation(resolvedTicket)"
                    >
                      <i
                        class="hide-on-print fa-solid fa-arrow-rotate-left text-slate-400 dark:text-white/70"
                      ></i>
                    </button>
                  </td>
                </tr>
