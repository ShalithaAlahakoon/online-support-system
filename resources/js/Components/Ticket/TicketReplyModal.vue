...
const replyTicket = async () => {
  try {
    form.post(route("ticket.page.reply"), {
      ...
      onSuccess: () => {
        router.visit(route("ticket", { entries: 10, page: 1 })), form.reset();
        notify(
          {
            group: "success",
            title: "Success",
...
<template>
  <form @submit.prevent="replyTicket">
    <div
      class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 common-scroll-bar"
        ...
        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
          ...
          <div class="w-full mt-3">
            <div class="">
              ...
              <Textarea
                id="Reply"
                v-model="form.ticket_reply_message"
                type="text"
                class="mt-1 block w-full h-50"
                autocomplete="reply"
                placeholder="Message"
              />
